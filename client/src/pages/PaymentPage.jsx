...
const PaymentPage = () => {
    ...
    const submitPaymentHandler = async () => {
        ...
        try {
            ...
            await axios.post(`/api/courses/${id}/enroll`, { paymentScreenshot }, config);
            setEnrolling(false);
            alert('Payment submitted successfully! You are now enrolled.');

            // First we replace the current 'Payment' page with the 'Home' page in the history stack
            navigate('/', { replace: true });

            // Then immediately push 'My Courses' on top.
            // This way, if they hit their browser's Back button, they go to Home!
            setTimeout(() => {
                navigate('/my-courses');
            }, 50);
        } catch (err) {
        ...
        <div className="pt-32 pb-20 min-h-screen bg-white dark:bg-darkBg transition-colors duration-300">
            <div className="container mx-auto px-6 max-w-3xl">
                <div className="bg-background dark:bg-darkCard rounded-3xl p-8 sm:p-12 shadow-sm border border-gray-100 dark:border-gray-800 text-center">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 mb-8 font-medium">You are buying: <span className="text-primary dark:text-accent font-bold">{course.title}</span> for ${course.price}</p>

                    <div className="flex justify-center mb-8">
                        <div className="bg-white p-4 rounded-xl shadow-md inline-block border-2 border-gray-100">
                            {/* Generic sample QR code, replace url if you have an actual UPI QR */}
                            <img src="https://upload.wikimedia.org/wikipedia/commons/d/d0/QR_code_for_mobile_English_Wikipedia.svg" alt="UPI QR Code" className="w-48 h-48 sm:w-64 sm:h-64 object-contain" />
                        </div>
                    </div>
                    ...
                    <div className="max-w-md mx-auto bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 text-left">
                        <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-3 text-center">Choose File / Upload Screenshot Here</label>
