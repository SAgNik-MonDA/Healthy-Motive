...
const ManageEbookEnrollments = () => {
    ...
    useEffect(() => {
        const fetchEnrollments = async () => {
            ...
            try {
                ...
                const { data } = await axios.get('/api/ebooks/enrollments/all', config);
                setEnrollments(data);
                setLoading(false);
            } catch (err) {
    ...
    if (loading) return <div>Loading e-book enrollments...</div>;
    ...
    return (
        <div className="bg-white dark:bg-darkCard rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
            <div className="px-8 py-6 border-b border-gray-100 dark:border-gray-800">
                <h2 className="text-xl font-bold text-gray-900 dark:text-white">E-Book Enrollments</h2>
            </div>
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-gray-50 dark:bg-gray-900/50 text-gray-600 dark:text-gray-400 text-sm uppercase tracking-wider">
                            ...
                            <th className="px-8 py-4 font-semibold">E-Book Name</th>
                        ...
                        {enrollments.map((record) => (
                            <tr key={record._id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/50">
                                ...
                                <td className="px-8 py-4 text-gray-600 dark:text-gray-300">
                                    {record.ebook?.title || 'Unknown E-Book'}
                                </td>
...
export default ManageEbookEnrollments;
