...
const ManageGuidedAudios = () => {
    ...
    const [formData, setFormData] = useState({
        title: '', description: '', price: 0, imageUrl: '', audioUrl: '',
        audioIncludes: ['🎧 High quality audio', '♾️ Full lifetime access', '📱 Listen on PC, Mobile, or Tablet', '✨ Instant access after purchase']
    });
    ...
    const handleAudioIncludeChange = (index, value) => {
        const newIncludes = [...formData.audioIncludes];
        const parsed = parseInclude(newIncludes[index], index);
        newIncludes[index] = `${parsed.icon} ${value}`;
        setFormData({ ...formData, audioIncludes: newIncludes });
    };

    const addAudioInclude = () => setFormData({ ...formData, audioIncludes: [...formData.audioIncludes, ''] });

    const removeAudioInclude = (index) => {
        setFormData({ ...formData, audioIncludes: formData.audioIncludes.filter((_, i) => i !== index) });
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const payload = {
                ...formData,
                audioIncludes: formData.audioIncludes.filter((inc, idx) => parseInclude(inc, idx).text.trim() !== '')
            };
            if (isEditing) {
                await axios.put(`/api/guidedaudios/${currentId}`, payload, config);
            } else {
                await axios.post('/api/guidedaudios', payload, config);
            }
            setShowModal(false);
            fetchAudios();
            setSuccessMsg(`Audio successfully ${isEditing ? 'updated' : 'created'}!`);
    ...
    const handleEdit = (audio) => {
        ...
        setFormData({
            ...
            audioIncludes: Array.isArray(audio.audioIncludes) && audio.audioIncludes.length > 0 ? audio.audioIncludes : ['']
        });
        setShowModal(true);
    };

    const handleDelete = async (id) => {
        if (window.confirm('Delete this Guided Audio?')) {
            try {
                const config = { headers: { Authorization: `Bearer ${user.token}` } };
                await axios.delete(`/api/guidedaudios/${id}`, config);
                fetchAudios();
                setSuccessMsg('Audio successfully deleted!');
                setTimeout(() => setSuccessMsg(''), 3000);
            } catch (err) {
                alert(err.response?.data?.message || err.message);
            }
    ...
    return (
        <div className="bg-white dark:bg-darkCard rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
            <div className="px-8 py-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center">
                <h2 className="text-xl font-bold text-gray-900 dark:text-white">Manage Guided Audios</h2>
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                        {audios.map((audio) => (
                            <tr key={audio._id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/50">
                                <td className="px-8 py-4">
                                    <div className="flex items-center gap-3">
                                        <div className="w-12 h-12 rounded bg-gray-200 overflow-hidden">
                                            {audio.imageUrl ? <img src={audio.imageUrl} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full bg-primary/20 flex items-center justify-center text-xl">🎧</div>}
                                        </div>
                                        <span className="font-semibold text-gray-900 dark:text-white">{audio.title}</span>
                                    </div>
                                </td>
                                <td className="px-8 py-4 text-gray-600 dark:text-gray-300">${audio.price}</td>
                                <td className="px-8 py-4 text-gray-500 text-sm">{new Date(audio.createdAt).toLocaleDateString()}</td>
                                <td className="px-8 py-4">
                                    <div className="flex gap-3">
                                        <button onClick={() => handleEdit(audio)} className="text-blue-500 hover:text-blue-700 bg-blue-50 dark:bg-blue-900/20 p-2 rounded-full transition-colors"><Edit size={18} /></button>
                                        <button onClick={() => handleDelete(audio._id)} className="text-red-500 hover:text-red-700 bg-red-50 dark:bg-red-900/20 p-2 rounded-full transition-colors"><Trash2 size={18} /></button>
                                    </div>
                                </td>
                            </tr>
                        ))}
                ...
                {audios.length === 0 && <div className="p-8 text-center text-gray-500">No guided audios found. Create one above!</div>}
            </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                    <div className="bg-white dark:bg-darkCard w-full max-w-2xl rounded-2xl shadow-xl flex flex-col max-h-[90vh]">
                        <div className="flex justify-between items-center p-6 border-b border-gray-100 dark:border-gray-800 shrink-0">
                            <h3 className="text-xl font-bold text-gray-900 dark:text-white">{isEditing ? 'Edit Audio' : 'Create Audio'}</h3>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-col gap-2">
                                        <input type="text" name="imageUrl" value={formData.imageUrl} onChange={handleChange} className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary outline-none" placeholder="/images/my-audio.jpg" />
                                ...
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">Audio File URL or Upload</label>
                                    <div className="flex flex-col gap-2">
                                        <input type="text" name="audioUrl" value={formData.audioUrl} onChange={handleChange} className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary outline-none" placeholder="/files/my-audio.mp3" />
                                        <input type="file" onChange={(e) => uploadFileHandler(e, 'audioUrl')} accept="audio/*" className="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary file:text-white hover:file:bg-opacity-90 transition" />
                                    </div>
                                </div>

                                {/* Audio Includes */}
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Audio Includes</label>
                                    <div className="space-y-3">
                                        {formData.audioIncludes.map((include, index) => {
                                            ...
                                            return (
                                                <div key={index} className="flex bg-white dark:bg-[#1e2330] border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-primary focus-within:border-transparent transition-all shadow-sm">
                                                    ...
                                                    <input
                                                        type="text"
                                                        value={text}
                                                        onChange={(e) => handleAudioIncludeChange(index, e.target.value)}
                                                    ...
                                                    />
                                                    {formData.audioIncludes.length > 1 && (
                                                        <button
                                                            type="button"
                                                            onClick={() => removeAudioInclude(index)}
                                    ...
                                    <button type="button" onClick={addAudioInclude} className="mt-4 flex items-center gap-1 text-sm font-bold text-primary dark:text-accent hover:underline">
                                ...
                                <div className="pt-4 flex gap-4">
                                    ...
                                    <button type="submit" className="flex-1 py-3 bg-primary text-white rounded-xl font-bold hover:bg-opacity-90 transition">{isEditing ? 'Save Changes' : 'Create Audio'}</button>
                                </div>
                            </form>
                        </div>
                    </div>
...
export default ManageGuidedAudios;
